...
namespace Faded_Injector.Handlers
{
    public class Spoof
    {
        readonly static string McDirectory = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + @"\Packages\Microsoft.MinecraftUWP_8wekyb3d8bbwe\LocalState\games\com.mojang\minecraftpe\";
        public static string DidPtr;
        public static string McidPtr;
        ...
        public static void AttachProcess()
        {
            ...
            {
                MessageBox.Show("Couldn't find process Minecraft.Windows");
            }
        ...
        public static void PatchMemory(string pointer, string dataType, string newValue)
        {
            try
            {
                AttachProcess();
                m.WriteMemory("base+" + pointer, dataType, newValue, "", null, true);
            } catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Patching Error");
            }
        }

        private static string RandomMcid()
        {
        ...
        public static async void DoTheSpoof(string gameVersion)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("Minecraft Directory not found, are you using a custom launcher or not have the game installed?");
                }
                else
                {
                ...
                if (m.OpenProcess("Minecraft.Windows"))
                {
                    ...
                    {
                        case "Latest":
                            DidPtr = await GetString("https://raw.githubusercontent.com/FadedKow/Assets/main/Other/DID.txt");
                            McidPtr = await GetString("https://raw.githubusercontent.com/FadedKow/Assets/main/Other/MCID.txt");
                            break;

                        case "1.19.40":
                            DidPtr = "048F1D70,0,20,0";
                            McidPtr = "0497D460,0,30,18,50,0";
                            break;

                        case "1.19.20":
                            DidPtr = "0454DF10,0,20,0";
                            McidPtr = "045B7760,10,30,20,60,0";
                            break;

                        case "1.19.11":
                            DidPtr = "043AB660,0,20,0";
                            McidPtr = "0440CF10,0,30,18,50,0";
                            break;

                        case "1.19.10":
                            DidPtr = "043AB610,0,20,0";
                            McidPtr = "0440CED0,0,30,18,50,0";
                            break;

                        case "1.19.2":
                            DidPtr = "0421EB78,0,20,0";
                            McidPtr = "0427DB00,0,30,18,50,0";
                            break;

                        case "1.19.0":
                            DidPtr = "041C20B8,0,20,0";
                            McidPtr = "04221030,0,30,20,60,0";
                            break;

                        case "1.18.31":
                            DidPtr = "0546B0B8,0,20,0";
                            McidPtr = "054C0DE0,0,30,18,10,60,0";
                            break;

                        case "1.18.12":
                            DidPtr = "0458F1A8,0,20,0";
                            McidPtr = "045CC590,0,30,20,60,0";
                            break;

                        case "1.17.11":
                            DidPtr = "040776C8,3F8,218,50,2F0,0";
                            break;

                        case "1.16.201":
                            DidPtr = "03A17598,10,80,0,8,290,0";
                            break;
                    }

                    if (!string.IsNullOrEmpty(DidPtr)) PatchMemory(DidPtr, "string", NewGuid.ToString());
                    if (!string.IsNullOrEmpty(McidPtr)) PatchMemory(McidPtr, "string", RandomMcid());
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Spoofing Error");
            }
        }
